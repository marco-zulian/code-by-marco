...
const strengthDisplay = document.querySelector("#strength-display") as StrengthDisplay;
...
function submitForm(e: Event) {
  ...
  if (password.length <= 8) { strengthDisplay.strength = 'Too Weak' }
  else if (password.length <= 15 ) { strengthDisplay.strength = 'Weak' }
  else {
...
function generatePassword(options: FormData): string {
  ...
  if (options.get('lower') !== null ) { characters.push(...lowercases) }
  if (options.get('upper') !== null ) { characters.push(...uppercases) }
  if (options.get('numbers') !== null ) { characters.push(...numbers) }
  if (options.get('symbols') !== null ) { characters.push(...symbols) }
